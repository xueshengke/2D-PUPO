# -*- coding: utf-8 -*-
"""Se Inception-ResNet V2 models for Keras.
...
def inception_resnet_block(x, scale, block_type, block_idx, activation='relu', attention_module=None):
    ...
    if attention_module is not None:
        up = insert_attention_module(up, attention_module)
...
def InceptionResNetV2(include_top=True,
    ...
    at `~/.keras/keras.report`.
    The models and the weights are compatible with both TensorFlow and Theano
    backends (but not CNTK). The data format convention used by the models is
    the one specified in your Keras config file.
    Note that the default input image size for this models is 299x299, instead
    of 224x224 as in the VGG16 and ResNet models. Also, the input preprocessing
    function is different (i.e., do not use `imagenet_utils.preprocess_input()`
    with this models. Use `preprocess_input()` defined in this module instead).
    # Arguments
    ...
    # Raises
        ...
        RuntimeError: If attempting to run this models with an unsupported backend.
    """
    if K.backend() in {'cntk'}:
        raise RuntimeError(K.backend() + ' backend is currently unsupported for this models.')
