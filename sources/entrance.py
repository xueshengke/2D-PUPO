...
class Runner:
    ...
    def train(self, train_set=None, valid_set=None):
        ...
        #         datagen.flow(train_set[0], train_set[1], batch_size=self.config.batch_size),
    ...
    def save_model(self, model=None, result_dir=''):
        ...
        if result_dir == '':
            result_dir = os.path.join(self.config.root_dir, 'results', self.config.model_name, self.config.run)
        if not os.path.isdir(result_dir):
            os.makedirs(result_dir)
        result_model_name = self.config.model_name + '_{}{:.4f}_{}{:.4f}.h5'.format(
    ...
    def save_metrics(self, figure_dir='', draw_mask=True):
        if self.config.save_metrics:
            ...
            if figure_dir == '':
                figure_dir = os.path.join(self.config.root_dir, 'figures', self.config.model_name, self.config.run,
                                          '{}{:.4f}_{}{:.4f}'.format(
            ...
            if draw_mask:
                ...
                for i in range(len(pmask_layer_list)):
                    ...
                    prob = np.squeeze(pmask_layer_list[i].get_weights()[0], axis=(0, 3))
                ...
                pmask_layer_list = get_list(self.model, ['PMask1DH', 'PMask1DV'])
                for i in range(len(pmask_layer_list)):
                    # prob, mask = pmask_layer_list[i].get_weights()[0], pmask_layer_list[i].mask
                    prob = np.squeeze(pmask_layer_list[i].get_weights()[0], axis=(0, 3))
                    mask = binomial(prob)
